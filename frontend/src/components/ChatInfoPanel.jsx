...
export default function ChatInfoPanel({
  ...
  return (
    <aside className="w-72 shrink-0 bg-white border-l border-sky-100 flex flex-col h-full relative md:w-80">
      ...
      <button
        onClick={onClose}
        className="md:hidden absolute top-4 left-4 size-8 rounded-lg flex items-center justify-center text-sky-400 hover:bg-sky-50 hover:text-sky-600 transition-colors cursor-pointer z-10"
      >
        <X className="size-4" />
      </button>
      {/* Header */}
      <div className="px-4 py-4 border-b border-sky-50 flex items-center justify-between md:justify-center">
        <span
          ...
          <div>
            ...
            <div className="space-y-1">
              {contact.members.map((m) => (
                <div
                  key={m._id}
                  className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-sky-50"
                >
                  ...
                  {m._id?.toString() !== currentUserId?.toString() && (
                    <button
                      onClick={() =>
                        act(`rm_${m._id}`, () =>
                          onRemoveMember(contact.id, m._id),
                        )
                      }
